    ...
    private static void EnsureSchema(OpenApiDocument document, string name)
    {
        ...
        if (!document.Components.Schemas.ContainsKey(name))
        {
            var description = name switch
            {
                ...
                "OutputResponse" => "Output aggregate: { state Ready|Generating|Failed|Partial|Unavailable, reason? (NO_RUNS_YET), completeness{ready,total}, progressApproximate? (Generating), errorCode? (Failed), items{video?,audio?,subtitles[],transcript?,translation?,timeline?,speakers?,qc{summary,issuesUrl?}}, warnings[], updatedAt } (signed URLs ≤15min only, never storage keys).",
                "ExportResponse" => "Export job: { id (exp_), projectId, format, status, isPartial, createdAt, completenessJson? } (incomplete without allowPartial → 409 EXPORT_INCOMPLETE/OUTPUT_INCOMPLETE; download 302 to ≤15min signed URL, 409 EXPORT_NOT_READY, 410 URL_EXPIRED).",
                "NotificationResponse" => "Notification row: { id (ntf_), type, severity, title, body (summary only), resourceType, resourceId, projectId?, readAt?, createdAt, expiresAt? } (newest-first, expired excluded; read/read-all idempotent).",
