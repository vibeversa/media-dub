    ...
    public static void AddDubbingOpenApi(IServiceCollection services)
    {
        ...
        services.AddOpenApi("v1", options =>
        {
            options.AddDocumentTransformer((document, _, _) =>
            {
                ...
                document.Info.Description = "Tenant-scoped dubbing pipeline. Auth: JWT bearer (tid/sub/roles) or ?access_token= on SSE /stream (same policy, never logged). Mutations accept Idempotency-Key. Errors use { error: { code, message, correlationId, details } } with codes including LANGUAGE_IMMUTABLE, SETTINGS_LOCKED_ACTIVE_RUN, PROJECT_NOT_FOUND, PROJECT_HAS_ACTIVE_RUN, SETTINGS_VERSION_CONFLICT, IDEMPOTENCY_KEY_REQUIRED, IDEMPOTENCY_KEY_REUSED, PROJECT_ARCHIVED, RUN_ALREADY_TERMINAL, RUN_ALREADY_ACTIVE, CONFIG_CHANGED_SINCE_RUN, SELECTION_CONFLICT, VERSION_NOT_FOUND, VERSION_SEGMENT_MISMATCH, SEGMENT_TEXT_EMPTY, SEGMENT_RETRY_ACTIVE, VOICE_INCOMPATIBLE, VOICE_CONSENT_REQUIRED, PREVIEW_QUOTA_EXCEEDED, VOICE_NOT_FOUND, PREVIEW_TEXT_INVALID, REVIEW_VERSION_CONFLICT, REVIEW_REASON_REQUIRED, REVIEW_ALREADY_RESOLVED, REVIEW_NOT_RESOLVED, REVIEW_EDIT_EMPTY. Project lists use { items, page, pageSize, total, sort, sortDir, hasMore, clamped }; dashboard summary is GET /api/v1/dashboard/summary with seven sections; processing start returns 202 (replay 200 + Idempotent-Replayed:true) and run lists use { items, page, pageSize, total, hasMore }; workspace aggregate is GET /api/v1/projects/{projectId}/workspace (single call, ten sections); progress exposes percentApproximate (display-only) and SSE is GET .../progress/stream (text/event-stream, Last-Event-ID accepted, event freeze in Task 013); segments support GET .../segments (filters speakerId/reviewStatus/qualityFlag/syncIssue/text/startMs/endMs, default 50/max 200, sort startMs) plus detail with versions and POST .../segments/{id}/retry|transcript-selection|translation-selection|transcript-edits|translation-edits (stale → 409 SELECTION_CONFLICT with { currentSelectionVersion, currentVersionIds }); speakers support GET .../speakers (segmentCount + assignedVoice) plus detail, GET .../speakers/{id}/available-voices (compatible-only + excludedCount/reasons), PUT .../speakers/{id}/voice-assignment { voiceId, reason? } (incompatible → 422 VOICE_INCOMPATIBLE, no consent → 403 VOICE_CONSENT_REQUIRED, same → 200 changed:false), and POST|GET .../voice-previews (202 first, 200 duplicate, quota → 429 PREVIEW_QUOTA_EXCEEDED, text → 400 PREVIEW_TEXT_INVALID); reviews support GET .../reviews/{id}/context (eleven sections: item, project, run, segment, versions, voice, audio, sync, qc, actions/permissions, history) plus POST .../reviews/{id}/resolve|dismiss|reopen|resolve-with-edit { expectedVersion, reason, editText? } + Idempotency-Key (stale → 409 REVIEW_VERSION_CONFLICT with { currentVersion }, missing reason → 400 REVIEW_REASON_REQUIRED, replay → 200 + Idempotent-Replayed:true, resolve-with-edit creates a manual version).";
